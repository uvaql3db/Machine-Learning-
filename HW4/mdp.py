...
#         pass
    ...
    def epsilon_greedy_policy(self, Q_current_state, epsilon,state):
...
#             print(np.random.randint(self.env.nA))
#             print("np.random.randint(state)",np.random.randint(state))
            return np.random.randint(state)
        else:
            print(np.argmax(Q_current_state.round(decimals=2)))
    ...
    def policy_iteration(self, iterations_to_save=[0], visualize=False):
        ...
        while iteration <= self.max_iterations and improve_policy:
            ...
            end_pe = time.time()
            total_pe = end_pe - start_pe
            print("policy evaluation time:", total_pe)
            ...
            end_ip = time.time()
            total_ip = end_ip - start_ip
            print("improve policy time:", total_ip)
...
#                 print("epsilon:", epsilon)
                action = self.epsilon_greedy_policy(Q[state], epsilon,state)  # choose an action with an epsilon-greedy policy
                print("action:",action)
#                 new_state, reward, done, _ = self.env.step(190) 
#                 print("haha")
                new_state, reward, done, _ = self.env.step(action)  # step to the new state
